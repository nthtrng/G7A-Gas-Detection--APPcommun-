...
$stmt2 = $pdo->query("SELECT * FROM historique_capteur_g7b_recul ORDER BY date_evenement DESC LIMIT 20");
...
<head>
  ...
  <title>G7B - Distance Sensor</title>
...
<nav class="navbar">
  ...
  <div class="nav-links">
    ...
    <a href="dashboard_g7b.php" class="active">G7B</a>
...
<div class="page">
  ...
  <div class="page-header">
    <div>
      <p class="page-title">G7B - Distance Sensor</p>
      <p class="page-sub">Last update: <?= $last ? date('d F Y \a\t H:i:s', strtotime($last['date_evenement'])) : 'No data' ?></p>
    </div>
    <div style="display:flex; gap:8px;">
      <a href="dashboard.php" class="btn btn-secondary"><i class="ti ti-arrow-left" style="font-size:14px"></i> Overview</a>
      <a href="dashboard_g7b.php" class="btn btn-primary"><i class="ti ti-refresh" style="font-size:14px"></i> Refresh</a>
    </div>
  </div>
  ...
  <div class="metrics">
    <div class="metric">
      ...
      <div class="metric-value"><?= h($last['distance_cm']) ?> <span style="font-size:13px;color:#aaa;">cm</span></div>
    </div>
    <div class="metric">
      <div class="metric-top"><span class="metric-label">Raw value</span><i class="ti ti-chart-line metric-icon"></i></div>
      <div class="metric-value"><?= h($last['valeur_brute']) ?></div>
    </div>
    <div class="metric">
      <div class="metric-top"><span class="metric-label">Status</span><i class="ti ti-shield metric-icon"></i></div>
      <div style="margin-top:6px;"><span class="badge badge-warn"><?= h($last['statut']) ?></span></div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="row">
      ...
      <span class="row-value"><?= h($last['distance_cm']) ?> cm</span>
    </div>
    <div class="row">
      <span class="row-label"><i class="ti ti-activity" style="font-size:14px"></i> Raw value</span>
      <span class="row-value"><?= h($last['valeur_brute']) ?></span>
    </div>
    <div class="row">
      <span class="row-label"><i class="ti ti-alert-triangle" style="font-size:14px"></i> Status</span>
      <span class="badge badge-warn"><?= h($last['statut']) ?></span>
    </div>
    <div class="row">
      <span class="row-label"><i class="ti ti-clock" style="font-size:14px"></i> Timestamp</span>
      <span class="row-value" style="font-size:12px;"><?= date('d F Y \a\t H:i:s', strtotime($last['date_evenement'])) ?></span>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <table>
      <thead>
        <tr><th>Timestamp</th><th>Distance</th><th>Raw value</th><th>Status</th></tr>
      </thead>
      <tbody>
        <?php foreach ($history as $row): ?>
        <tr>
          <td style="color:#888;"><?= date('d F Y \a\t H:i:s', strtotime($row['date_evenement'])) ?></td>
          <td><?= h($row['distance_cm']) ?> cm</td>
          <td><?= h($row['valeur_brute']) ?></td>
          <td><span class="badge badge-warn"><?= h($row['statut']) ?></span></td>
        </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
